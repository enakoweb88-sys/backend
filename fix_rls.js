...
async function run() {
  ...
  try {
    ...
    console.log("Creating permissive policies to fix production crash...");
    for (const table of tables) {
      try {
        await client.query(`
          DROP POLICY IF EXISTS "allow_all_public" ON "public"."${table}";
          CREATE POLICY "allow_all_public" ON "public"."${table}" FOR ALL USING (true) WITH CHECK (true);
        `);
        console.log(`Successfully added policy for "public"."${table}"`);
      } catch (e) {
